...
本記事では、**helix-codex**というMCPサーバーを使って、Claude CodeからGPT-5.4にタスクを委譲し、構造化された実行レポートを受け取る方法をハンズオンで解説します。
...
`/path/to/helix-codex` は実際のクローン先パスに置き換えてください。
...
```
helix-codexのexecuteツールを使って、src/auth.pyの認証ロジックを修正してください
```
...
```
helix-codexのreviewツールで、さっき書いたコードをレビューしてください
```
...
```
helix-codexのparallel_executeで以下の3ファイルを同時にセキュリティレビューしてください:
- src/auth.py
- src/db.py
- src/api.py
```
...
helix-codexは3段階のサンドボックスモデルを採用しています:
...
helix-codexを使えば、MCPプロトコルを介してClaude CodeとGPT-5.4を実用的に連携させることができます。
...
リポジトリ: https://github.com/tsunamayo7/helix-codex
ライセンス: MIT
