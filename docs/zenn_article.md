---
title: "【Claude Code×GPT-5.4】AIがAIをレビューする時代 — claude-code-codex-agentsでAdversarial Review Loopを実装した"
emoji: "🔗"
type: "tech"
...
[claude-code-codex-agents](https://github.com/tsunamayo7/claude-code-codex-agents) は、Claude CodeからCodex CLI (GPT-5.4) をMCPツールとして呼び出し、**実行過程を構造化レポートとして返す**MCPサーバーだ。ただのプロキシではない。JSONLイベントストリームを完全解析し、ツール使用・ファイル操作・タイミング・エラーをすべて可視化する。
...
claude-code-codex-agentsの核心は、Codex CLIが吐く**JSONLイベントストリームの完全解析**にある。
...
## Adversarial Review Loop：GPT-5.4がclaude-code-codex-agents自身をレビュー

ここからが本題。**claude-code-codex-agents自身のコード（server.py）をGPT-5.4にレビューさせた。**
...
claude-code-codex-agentsは3層のサンドボックスポリシーを実装している。
...
Codex CLIは`threadId`でセッションを管理する。claude-code-codex-agentsはこれを`SessionManager`で追跡し、後続の呼び出しで前回のコンテキストを引き継げる。
...
GitHub上にはCodex MCPブリッジが6つ以上存在する。claude-code-codex-agentsの差別化ポイント：
...
claude-code-codex-agentsで実現したのは、単なるAPI橋渡しではない。
...
GPT-5.4がclaude-code-codex-agents自身のCRITICAL 3件を検出した事実は、**単一モデルでの開発には限界がある**ことを示している。AIがAIをレビューする時代は、すでに始まっている。
